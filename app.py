...
st.caption("Dobór osprzętu modułowego (gniazda, łączniki, ramki) oraz wycena robocizny")

# 1. DANE ZLECENIA
with st.expander("1. Dane zlecenia i seria osprzętu", expanded=True):
    col_d1, col_d2 = st.columns(2)
    with col_d1:
        firma = st.text_input("Wykonawca:", value="Elektro-Instal")
        klient = st.text_input("Inwestor:", value="Jan Nowak")
    with col_d2:
        nr_oferty = st.text_input("Numer oferty:", value="BM/2026/01")
        adres = st.text_input("Adres inwestycji:", value="ul. Modrzewiowa 12")

    col_s1, col_s2 = st.columns(2)
    with col_s1:
        seria_osprzetu = st.selectbox(
            "Seria osprzętu:",
            ["Kontakt-Simon 54 Premium", "Kontakt-Simon 10", "Schneider Sedna Design", "Legrand Niloe Step"]
        )
    with col_s2:
        kolor_osprzetu = st.selectbox(
            "Kolor mechanizmów i ramek:",
            ["Biały", "Antracyt mat", "Czarny mat", "Srebrny mat", "Kremowy", "Inox / Stal"]
        )

# Domyślne stawki robocizny
with st.expander("Stawki robocizny (montaż)", expanded=False):
    robocizna_stawka_modul = st.number_input("Montaż 1 mechanizmu/gniazda w puszce (zł):", value=25.0, step=2.0)
    robocizna_stawka_lampa = st.number_input("Montaż lampy / żyrandola (zł):", value=75.0, step=5.0)

# 2. GNIAZDA
with st.expander("2. Gniazda wtykowe i specjalne", expanded=False):
    cg1, cg2 = st.columns(2)
    q_g1 = cg1.number_input("Gniazdo pojedyncze 230V z uziemieniem (szt.):", value=30, step=1)
    p_g1 = cg2.number_input("Cena mat. gniazdo 1x (zł):", value=16.50, step=1.0)

    q_g2 = cg1.number_input("Gniazdo podwójne do ramki (szt.):", value=10, step=1)
    p_g2 = cg2.number_input("Cena mat. gniazdo 2x (zł):", value=24.00, step=1.0)

    q_g_ip = cg1.number_input("Gniazdo z klapką IP44 (szt.):", value=4, step=1)
    p_g_ip = cg2.number_input("Cena mat. gniazdo IP44 (zł):", value=22.00, step=1.0)

    q_g_usb = cg1.number_input("Gniazdo z ładowarką 2xUSB A+C (szt.):", value=2, step=1)
    p_g_usb = cg2.number_input("Cena mat. gniazdo USB (zł):", value=115.00, step=5.0)

    q_g_sila = cg1.number_input("Gniazdo siłowe 16A/32A (szt.):", value=1, step=1)
    p_g_sila = cg2.number_input("Cena mat. gniazdo siłowe (zł):", value=65.00, step=5.0)

# 3. ŁĄCZNIKI
with st.expander("3. Łączniki i sterowanie", expanded=False):
    cl1, cl2 = st.columns(2)
    q_sw1 = cl1.number_input("Łącznik pojedynczy 1-biegunowy (szt.):", value=6, step=1)
    ...
    q_sw_sch = cl1.number_input("Łącznik schodowy / krzyżowy (szt.):", value=4, step=1)
...
# 4. TELETECHNIKA
with st.expander("4. Gniazda teletechniczne (LAN, TV)", expanded=False):
    ct1, ct2 = st.columns(2)
    q_rj45 = ct1.number_input("Gniazdo komputerowe RJ45 kat. 6 (szt.):", value=6, step=1)
    p_rj45 = ct2.number_input("Cena mat. gniazdo RJ45 (zł):", value=32.00, step=2.0)

    q_tv = ct1.number_input("Gniazdo R-TV-SAT końcowe (szt.):", value=3, step=1)
...
with st.expander("5. Ramki wielokrotne", expanded=False):
    ...
    q_r2 = cr1.number_input("Ramka 2-krotna (szt.):", value=10, step=1)
    p_r2 = cr2.number_input("Cena ramki 2x (zł):", value=11.50, step=1.0)

    q_r3 = cr1.number_input("Ramka 3-krotna (szt.):", value=6, step=1)
...
with st.expander("6. Montaż lamp i oświetlenia", expanded=False):
    co1, co2 = st.columns(2)
    q_lampa_wisz = co1.number_input("Montaż lampy wiszącej / żyrandola (szt.):", value=4, step=1)
    p_lampa_wisz = co2.number_input("Stawka montażu lampy (zł):", value=robocizna_stawka_lampa, step=5.0)

    q_spot = co1.number_input("Montaż oczka / oprawy LED (szt.):", value=12, step=1)
    p_spot = co2.number_input("Stawka montażu oczka (zł):", value=30.0, step=5.0)

    q_kinkiet = co1.number_input("Montaż kinkietu ściennego (szt.):", value=3, step=1)
    p_kinkiet = co2.number_input("Stawka montażu kinkietu (zł):", value=50.0, step=5.0)

    q_led_m = co1.number_input("Montaż taśmy LED w profilu [mb]:", value=8, step=1)
    p_led_m = co2.number_input("Stawka montażu LED (zł/mb):", value=40.0, step=5.0)

# PODATEK VAT
vat_dict = {"8% (Mieszkaniowy)": 0.08, "23% (Komercyjny / Faktura)": 0.23, "0% (Zwolnienie)": 0.00}
...
def generate_pdf():
    ...
    title_style = ParagraphStyle('DocTitle', parent=styles['Normal'], fontName=FONT_BOLD, fontSize=15, leading=19, textColor=colors.HexColor("#0f172a"))
    text_bold = ParagraphStyle('TBold', parent=styles['Normal'], fontName=FONT_BOLD, fontSize=8.5, leading=11)
    ...
    elements.append(Paragraph(f"Seria: <b>{seria_osprzetu}</b> | Kolor: <b>{kolor_osprzetu}</b>", text_normal))
...
# WBUDOWANY PODGLĄD PDF W PRZEGLĄDARCE
with st.expander("👁️ Podgląd dokumentu PDF na ekranie", expanded=True):
